...
interface ProductAttribute {
  ...
  name: string;
  value: string;
  type: 'TEXT' | 'NUMBER' | 'BOOLEAN' | 'COLOR' | 'IMAGE' | 'SELECT' | 'MULTI_SELECT';
  isRequired: boolean;
...
export default function ProductForm({ isOpen, onClose, onSubmit, initialData, isLoading = false }: ProductFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    dimensions: initialData?.dimensions || { length: 0, width: 0, height: 0, unit: 'cm' },
  ...
  const addPricingTier = () => {
    if (newPricingTier.minQuantity && newPricingTier.price) {
      setPricingTiers(prev => [...prev, { ...newPricingTier, id: Date.now().toString() }]);
  ...
  const addAttribute = () => {
    if (newAttribute.name && newAttribute.value) {
      setAttributes(prev => [...prev, { ...newAttribute, id: Date.now().toString() }]);
      setNewAttribute({ name: '', value: '', type: 'TEXT', isRequired: false, isFilterable: true, sortOrder: 0 });
    }
  };
              ...
              <div className="space-y-6">
                ...
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-semibold text-gray-900">Dynamic Pricing Tiers</h3>
                    <button
                      type="button"
                  ...
                  <div className="space-y-4">
                    {pricingTiers.map((tier, index) => (
                      <div key={tier.id || index} className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 border border-gray-200 rounded-lg">
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              updatedTiers[index].minQuantity = parseInt(e.target.value) || 1;
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              const updatedTiers = [...pricingTiers];
                              updatedTiers[index].maxQuantity = parseInt(e.target.value) || undefined;
                              setPricingTiers(updatedTiers);
                            }}
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-black"
                        ...
                        <div className="flex items-end">
                          <button
                            type="button"
                            onClick={() => removePricingTier(tier.id || index.toString())}
                    ...
                    {pricingTiers.length === 0 && (
                      <div className="text-center py-8 text-gray-500">
                        ...
                        <p className="text-sm">Add tiers to offer different prices based on quantity</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center">
                    <input
                      ...
                      className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                    />
                    <label htmlFor="manageStock" className="ml-2 block text-sm text-gray-900">
                      Manage stock levels
                    </label>
                  </div>
                </div>
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    {formData.seoTitle.length}/60 characters. Recommended: 50-60 characters
                  </p>
                </div>
                ...
                <div>
                  ...
                  />
                  <p className="text-xs text-gray-500 mt-1">
                    {formData.seoDescription.length}/160 characters. Recommended: 150-160 characters
                  </p>
                </div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-2">
                    {formData.seoKeywords.map((keyword: string, index: number) => (
                      ...
                      >
                        ...
                        <button
                          type="button"
                          onClick={() => handleArrayChange('seoKeywords', keyword, 'remove')}
                    ...
                    <input
                      ...
                      onKeyPress={(e) => {
                        if (e.key === 'Enter') {
                          e.preventDefault();
                          if (newKeyword.trim()) {
                            handleArrayChange('seoKeywords', newKeyword.trim(), 'add');
                            setNewKeyword('');
                          }
                        }
                      }}
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  {attributes.map((attr, index) => (
                    <div key={attr.id || index} className="grid grid-cols-1 md:grid-cols-5 gap-4 p-4 border border-gray-200 rounded-lg">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">
                          Name
                        </label>
                      ...
                      <div className="flex items-end space-x-2">
                        ...
                        <label className="flex items-center">
                          <input
                            ...
                            onChange={(e) => {
                              const updatedAttrs = [...attributes];
                              updatedAttrs[index].isFilterable = e.target.checked;
                              setAttributes(updatedAttrs);
                            }}
                            className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                          />
                          <span className="ml-1 text-sm text-gray-700">Filterable</span>
                        </label>
                      </div>
                      <div className="flex items-end">
                        <button
                          type="button"
                          onClick={() => removeAttribute(attr.id || index.toString())}
                          className="w-full px-3 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700"
                        >
                          <Trash2 className="w-4 h-4 mx-auto" />
                        </button>
                  ...
                  {attributes.length === 0 && (
                    <div className="text-center py-8 text-gray-500">
                      ...
                      <p className="text-sm">Add attributes like color, size, material, etc.</p>
                    </div>
                  )}
                </div>
              </div>
