  ...
  const getTabDescription = (tabKey: TabKey): string => {
    switch (tabKey) {
      ...
      case "variant-management":
        return "Manage product variant settings and options";
      case "seo":
...
function VariantManagementSettings({ settings, onChange, onBooleanChange }: any) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <InputField
          ...
          onChange={(v: string) => onChange("defaultVariantImageWidth", Number(v))}
          placeholder="500"
        />
        <InputField
          ...
          onChange={(v: string) => onChange("defaultVariantImageHeight", Number(v))}
          placeholder="500"
        />
      </div>
      ...
      <div className="space-y-4">
        <CheckboxField
          ...
          onChange={(v: boolean) => onBooleanChange("variantAutoGeneration", v)}
        />
        <CheckboxField
          label="Allow Variant Combinations"
          checked={settings.allowVariantCombinations || true}
          onChange={(v: boolean) => onBooleanChange("allowVariantCombinations", v)}
        />
        <CheckboxField
          label="Show Out of Stock Variants"
          checked={settings.showOutOfStockVariants || false}
          onChange={(v: boolean) => onBooleanChange("showOutOfStockVariants", v)}
        />
        <CheckboxField
          label="Enable Variant Fallback"
          checked={settings.variantFallbackEnabled || true}
          onChange={(v: boolean) => onBooleanChange("variantFallbackEnabled", v)}
        />
      </div>
      ...
      <div>
        ...
        <TextAreaField
          value={settings.variantAttributeTypes || "Color,Size,Material,Package,Units"}
          onChange={(v: string) => onChange("variantAttributeTypes", v)}
          rows={3}
