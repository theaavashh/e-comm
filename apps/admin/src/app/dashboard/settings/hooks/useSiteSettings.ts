        ...
        if (data.data) {
          const saveResponse = await fetch(
            `${API_BASE_URL}/api/v1/configuration/site-settings`,
            {
              ...
              headers: getAuthHeaders(),
              body: JSON.stringify({
                [file.type.startsWith("image/") ? "siteLogo" : "siteFavicon"]:
                  data.data.url,
              }),
            },
          );
...
function getDefaultSettings(): SiteSettings {
  return {
    ...
    seoDescription:
      "Shop the latest products at Gharsamma Ecommerce. Quality products, competitive prices, and excellent customer service. Free delivery across Nepal.",
    seoKeywords:
      "online shopping, ecommerce, nepal, electronics, clothing, books, home, garden, beauty, health",
