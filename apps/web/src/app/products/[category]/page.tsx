    ...
    const load = async () => {
      try {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:4444"}/api/v1/products?category=${encodeURIComponent(String(category))}&limit=100&country=${selectedCountry || "USA"}`,
        );
        ...
        const list: ApiProduct[] = data.success ? data.data.products : [];

        setProducts(list);

        // Extract unique brands from API response
        const uniqueBrands = Array.from(
          new Set(
            (list || []).map((p) => p.brand?.name || "Unknown").filter(Boolean),
          ),
        );
