...
import {
  ...
  NonVegAcharContent,
  AllAcharContent,
} from "@/components/AcharComponents";
...
export default function FoodsPage() {
  ...
  const [selectedAcharType, setSelectedAcharType] = useState<
    "all" | "veg" | "non-veg"
  >("all");
  ...
  useEffect(() => {
    const fetchRecommendedProducts = async () => {
      try {
        setLoading(true);
        const apiUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/v1/products?categories=foods&limit=8`;
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        const categoryName = foodCategories
          .find((c) => c.id === selectedCategory)
          ?.name.toLowerCase();
        const apiUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL}/api/v1/products?categories=foods&subcategory=${categoryName}`;
          ...
          {selectedCategory === 0 ? (
            <div className="text-center py-20">
              ...
              <p className="text-gray-600">
                Choose from the categories on the left to explore our food
                products
              </p>
            </div>
          ) : (
                ...
                >
                  <div className="text-center">
                    ...
                    <div className="text-sm text-red-600">
                      Non-vegetarian pickle varieties
                    </div>
                  </div>
                </motion.button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  { name: "Radish", icon: "🥕", description: "Radish pickle" },
                  {
                    name: "Mushroom",
                    icon: "🍄",
                    description: "Mushroom pickle",
                  },
                  {
                    name: "Gundruk",
                    icon: "🥬",
                    description: "Fermented leafy greens",
                  },
                  {
                    name: "Timur Chop (Powder)",
                    icon: "🌶️",
                    description: "Timur powder pickle",
                  },
                  {
                    name: "Timur Chop (Oily)",
                    icon: "🫒",
                    description: "Oily timur pickle",
                  },
                ].map((item, index) => (
                  <motion.button
                    key={item.name}
